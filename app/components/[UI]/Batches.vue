...
type ActivityWithStatus = Activity & Partial<ValidatorEpochStatusResult>
...
function getTitle({ missed, rewarded, epochNumber, status, missRate }: Partial<ActivityWithStatus>) {
...
<template>
  <ul grid="~ cols-50 gap-4">
    <DefineBatchState v-slot="{ missed, rewarded, epochNumber, status, missRate, title }">
      <div
        ...
        :class="{
          ...
          'bg-red-500': status === ValidatorEpochStatus.ElectedFailedOrOffline,
