...
type NoNull<T> = Required<{ [K in keyof T]: Exclude<T[K], null> }>
export type KnownValidator = NoNull<Omit<NewValidator, 'id'>>

export const knownValidators: KnownValidator[] = [
  {
...
].map(v => ({ description: '', website: '', ...v }))

const defaultValidatorInfo: KnownValidator = {
  name: 'Unknown validator',
  address: '',
  fee: -1,
  icon: '',
  payoutType: PayoutType.Unknown,
  tag: ValidatorTag.Unknown,
  description: '',
  website: ''
}

export async function getKnownValidator(address: string) {
  let validator = knownValidators.find(v => v.address === address) || { ...defaultValidatorInfo, address }
  if (!validator.icon)
    validator.icon = await Identicons.default.toDataUrl(address) as string
  return validator
}
