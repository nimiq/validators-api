...
import type { Range, Result } from 'nimiq-validator-trustscore/types'
...
export async function extractRangeFromRequest(rpcClient: NimiqRPCClient, event: H3Event<EventHandlerRequest>): Result<Range> {
  ...
  if (currentEpochError || !currentEpoch)
    return { error: JSON.stringify(currentEpochError) }
  ...
  if (userEpoch === 'latest')
    ...
    return { error: `Epoch ${epoch} is in the future or it didn't finished yet. The newest epoch you can fetch is ${currentEpoch - 1}.` }
  else
    epoch = userEpoch
