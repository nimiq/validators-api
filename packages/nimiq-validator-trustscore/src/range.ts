...
import { BLOCK_SEPARATION_TIME, BLOCKS_PER_EPOCH, firstBlockOf } from '@nimiq/utils/albatross-policy'
...
export async function getRange(client: NimiqRPCClient, options?: GetRangeOptions): Result<Range> {
  ...
  const fromBlockNumber = firstBlockOf(fromEpoch)! - 1
  const toBlockNumber = firstBlockOf(toEpoch - 1)! - 1
